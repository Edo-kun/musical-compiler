/**************************************************************
 ...
 CUP bantam.parser integration improved according to suggestions made by
 David MacMahon <davidm@smartsc.com>.  The %cup directive now tells
 JLex to generate a bantam.parser conforming to the java_cup.runtime.Scanner
 ...
 bantam.lexer.
 Generated bantam.lexer now accepts unicode LS (U+2028) and PS (U+2029) as
 ...
 action code blocks.
 Fixed input bantam.lexer to allow whitespace in character classes, closing
 ...
 The bantam.lexer can now be initialized with either an InputStream
 ...
 sequences.  Added try/catch block around bantam.lexer generation
 ...
 Made the class "bantam.Main" instead of "JavaLex",
 improved the installation instructions to reflect this.

 Version 1.1, August 15, 1996  [Andrew Appel]
 Made yychar, yyline, yytext global to the bantam.lexer so that
...
/******************************
 ...
 regular expressions
 18) clean up all of the class files the bantam.lexer
...
class CSpec {
  ...
  /* Lexical States. */
	...
	/* Initialize hashtable for bantam.lexer states. */
	...
	/* Initialize variables for bantam.lexer options. */
...
class CMakeNfa {
     ...
     * ************************************************************
     * Function: machine
     * Description: Recursive descent regular expression bantam.parser.
     * ************************************************************
     */
...
class CNfa2Dfa {
	...
	/* Allocate mapping array. */
	    ...
	    /* CSA bugfix: if we skip all zero size rules, then
	       ...
	       bantam.lexer (0 states) instead of a bantam.lexer that rejects
	    ...
	    */
	    ...
	    /* bantam.Main loop of CDTrans creation. */
...
public class Main {
    ...
    (
        ...
        if (arg.length < 1) {
            System.out.println("Usage: JLex.bantam.Main <filename>");
            return;
        }
...
class CLexGen {
    ...
    private CSpec m_spec; /* Spec class holds information
			   about the generated bantam.lexer. */
    ...
    private CEmit m_emit; /* Output module that emits source code
			   into the generated bantam.lexer file. */
 ...
 *
 * Re-implementation of the standard java.bantam.util.BitSet to support sparse
